...
def writeTSPLIBfile(tsp_fname, CostMatrix, test_comment):
    ...
    with open((pwd + tsplib_dir + tsp_fname + '.tsp'), 'w', encoding='utf-8') as problem_file:
        ...
        problem_file.write('DISPLAY_DATA_TYPE: NO_DISPLAY\n')
        problem_file.write('EDGE_WEIGHT_SECTION' + '\n')
        for line in Cost_Matrix_STRline:
            problem_file.write(line)
        problem_file.write('EOF\n')

    with open((pwd + tsplib_dir + tsp_fname + '.par'), 'w', encoding='utf-8') as parameter_file:
